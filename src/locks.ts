...
const RUN_KEY_PREFIX = "weekly-cup-run-";
...
function r2Key(episodeIso: string): string {
  return `weekly-cup/${episodeIso}/run.json`;
}
...
export async function readRunRecord(
  ...
  if (env.WEEKLY_CUP_KV) {
    const v = await env.WEEKLY_CUP_KV.get(kvKey(episodeIso), "json");
    if (v) return v as RunRecord;
  }
  const obj = await env.WEEKLY_CUP_BUCKET.get(r2Key(episodeIso));
  if (!obj) return null;
  try {
...
export async function writeRunRecord(
  ...
  await env.WEEKLY_CUP_BUCKET.put(r2Key(record.episode_date), text, {
    httpMetadata: { contentType: "application/json; charset=utf-8" },
  });
  if (env.WEEKLY_CUP_KV) {
    await env.WEEKLY_CUP_KV.put(kvKey(record.episode_date), text, {
